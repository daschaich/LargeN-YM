...
int main(int argc, char *argv[]) {
  ...
  for(Intcount = 0; Intcount <= (int)((Emax-Emin)/delta); Intcount++) {
    ...
    for(Ncount = 0; Ncount < Njacknife; Ncount++) {
      ...
      for (traj_done = 0; traj_done < 5; traj_done++)
        update();
      node0_printf("WARMUPS COMPLETED\n");
      E = gauge_action();
      node0_printf("AFTER WARMUP E %.8g\n",
               E);
      ...
      constrained = 1;
      for (RMcount = RMstart; RMcount < ait; RMcount++) {
...
//            Nmeas++;
        ...
        if(accrate_it>0.4){
          if (RMcount < 20){
              ...
              a += 1.0*12.0 * Reweightexpect / (deltaSq * (RMcount - 20.0 + 1.0));
