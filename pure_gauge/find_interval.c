...
void findEint(double E_min) {
#ifdef LLR
  int Efound = -99;
  int counter = 0, count_max = 4000;
  double E, dtime, beta_sav = beta;
  //find smaller center
  double E_max = E_min + 1.0/1.0*delta;
  ...
  while (Efound < 0 && counter < count_max) {
    ...
    if (E >= E_min && E <= E_max) {
      ...
      if(abs(E-(E_max))>0.1*volume)
      {
        beta += 0.1;
      }
      else if (abs(E-E_max)>0.01*volume)
      {
        beta += 0.01;
      }
      else if (abs(E-E_max)>0.001*volume)
      {
        beta += 0.001;
      }
      else
      ...
      if(abs(E-(E_min))>0.1*volume)
      {
        beta -= 0.1;
      }
      else if (abs(E-E_min)>0.01*volume)
      {
        beta -= 0.01;
      }
      else if (abs(E-E_min)>0.001*volume)
      {
        beta -= 0.001;
      }
      else
