...
/*---------------------------------------------------------------------------*/
/* Convert a single precision wilson_vector to double precision */
  ...
  for(j=0,t=0;t<nt;t++)for(z=0;z<nz;z++)for(y=0;y<ny;y++)for(x=0;x<nx;x++,j++)
	...
	{	
	  if(this_node==currentnode){
	    i=node_index(x,y,z,t);
	    /* Copy and convert data into send buffer and send to node
	       0 with padding */
	    d2f_wvec((wilson_vector *)F_PT(&lattice[i],src),&msg.wv);
...
int r_serial_w(w_prop_file *wpf, int spin, int color, field_offset src)
{
  ...
  for(rcv_rank=0; rcv_rank<volume; rcv_rank++)
      ...
      if(this_node==destnode)
	{
	  ...
	  /* Convert to double precision and put in destination */
	  ...
	  if(this_node==currentnode)
	    {
	      l=node_index(x,y,z,t);
	      lbuf = *( (wilson_vector *)F_PT( &(lattice[l]), src ) );
	      send_field((char *)&lbuf,sizeof(wilson_vector),0);
	    }
	}
    }
  g_sync();
...
/* Since these files are certainly not archived, but more likely used
   as temporary extended storage, the file is in double precision */
