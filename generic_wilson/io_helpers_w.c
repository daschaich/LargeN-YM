...
w_prop_file *r_open_wprop(int flag, char *filename)
{
  ...
  if(this_node==0)
    ...
    case RELOAD_MULTIDUMP:
      printf("CAUTION: Reloaded propagator will be used for APBC only!\n");
    default:
      ;
    } 
#endif
...
w_prop_file *w_open_wprop(int flag, char *filename)
{
  ...
  if(this_node==0)
    ...
    case SAVE_CHECKPOINT:
      printf("CAUTION: Only the APBC propagator will be saved!\n");
    default:
      ;
    }
#endif
			  ...
			  field_offset dest, int timing)
{
  ...
  case FRESH:     /* zero initial guess */
    FORALLSITES(i,s){
      ...
      for(color = 0; color < DIMF; color++)for(spin = 0; spin < 4; spin++)
