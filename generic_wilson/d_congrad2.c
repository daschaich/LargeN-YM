/******* conjugate gradient for SU3/fermions ****/
/* Wilson fermions */
...
start:
	...
	*/
	rsq = source_norm = 0.0;
#ifdef LU
        dslash_w_site_special(F_OFFSET(psi),F_OFFSET(psi),PLUS,ODD, tag, 0 );
        dslash_w_site_special(F_OFFSET(psi),F_OFFSET(ttt),PLUS,EVEN,tag2,0 );
        FOREVENSITES(i,s){
            scalar_mult_add_wvec( &(s->psi),&(s->ttt), -kappa*kappa,&(s->ttt) );
        }
        dslash_w_site_special(F_OFFSET(ttt),F_OFFSET(ttt),MINUS,ODD,tag,1 );
        dslash_w_site_special(F_OFFSET(ttt),F_OFFSET(mp),MINUS,EVEN,tag2,1 );
        FOREVENSITES(i,s){
...
#else
	dslash_w_site_special(F_OFFSET(psi),F_OFFSET(ttt),PLUS,EVENANDODD,tag,0);
	FORALLSITES(i,s){
	    scalar_mult_add_wvec( &(s->psi), &(s->ttt), -kappa, &(s->ttt) );
	}
	dslash_w_site_special(F_OFFSET(ttt),F_OFFSET(mp),MINUS,EVENANDODD,tag,1);
	FORALLSITES(i,s){
...
iteration,(double)rsq,(double)pkp,(double)a );**/
	...
	*/
	...
	pkp = 0.0;
#ifdef LU
        dslash_w_site_special(F_OFFSET(p),F_OFFSET(p) ,PLUS,ODD,tag,1 );
        dslash_w_site_special(F_OFFSET(p),F_OFFSET(ttt),PLUS,EVEN,tag2,1);
        FOREVENSITES(i,s){
...
#else
	dslash_w_site_special(F_OFFSET(p),F_OFFSET(ttt),PLUS,EVENANDODD,tag,1);
	FORALLSITES(i,s){
