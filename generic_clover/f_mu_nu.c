...
void f_mu_nu(su3_matrix f_mn[], int mu, int nu) {
  ...
  FORALLSITES(i, s)
    mult_su3_an(&(s->link[nu]), &(s->link[mu]), &(tempmat[i]));
  ...
  FORALLSITES(i, s) {
    mult_su3_na((su3_matrix *)(gen_pt[0][i]), (su3_matrix *)gen_pt[1][i],
                &(f_mn[i]));
    mult_su3_nn(&(tempmat[i]), &(f_mn[i]), &(tempmat2[i]));
    mult_su3_nn(&(f_mn[i]), &(tempmat[i]), &(staple[i]));
  }
  ...
  tag3 = start_gather_field(staple, sizeof(su3_matrix),
                            OPP_DIR(mu), EVENANDODD, gen_pt[3]);

  /* Now make +mu +nu plaquette and put in f_mn */
  FORALLSITES(i, s) {
    mult_su3_nn(&(s->link[mu]), &(f_mn[i]), &tmat);
    mult_su3_na(&tmat, &(s->link[nu]), &(f_mn[i]));
  }

  /* Now gather +mu -nu plaquette and add to f_mn */
  wait_gather(tag2);
  FORALLSITES(i, s)
    sum_su3_matrix((su3_matrix *)(gen_pt[2][i]), &(f_mn[i]));

  cleanup_gather(tag2);
  FORALLSITES(i, s) {
    mult_su3_an((su3_matrix *)(gen_pt[1][i]), &(tempmat[i]), &tmat);
  ...
  wait_gather(tag3);
  FORALLSITES(i, s)
    sum_su3_matrix((su3_matrix *)(gen_pt[3][i]), &(f_mn[i]));
  cleanup_gather(tag3);

  /* Finally gather -mu -nu plaquette and add to f_mn */
  wait_general_gather(tag4);
  FORALLSITES(i, s)
    sum_su3_matrix((su3_matrix *)(gen_pt[4][i]), &(f_mn[i]));
  cleanup_general_gather(tag4);

  // Final factor of 1 / 8 on (f_mn - f_mn^dag)
  FORALLSITES(i, s) {
    su3_adjoint(&(f_mn[i]), &tmat);
    if (order_flag == 0)
      sub_su3_matrix(&(f_mn[i]), &tmat, &tmat);
    else
      dif_su3_matrix(&(f_mn[i]), &tmat);
    scalar_mult_su3_matrix(&tmat, 0.125, &(f_mn[i]));
  }
}
// -----------------------------------------------------------------
