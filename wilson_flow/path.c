...
void path(int *dir, int *sign, int length) {
  ...
  if (sign[0] > 0)  {
    mtag = start_gather_site(F_OFFSET(linkf[dir[0]]), sizeof(matrix),
  ...
  if (sign[0] < 0) {
    FORALLSITES(i, s)
      adjoint(&(s->linkf[dir[0]]), &(tempmat2[i]));
  }

  for (j = 1; j < length; j++) {
    if (sign[j] > 0) {
      FORALLSITES(i, s)
        mult_nn(&(tempmat2[i]), &(s->linkf[dir[j]]), &(tempmat[i]));
    ...
    if (sign[j] < 0) {
      ...
      wait_gather(mtag);
      FORALLSITES(i, s)
        mult_na((matrix *)(gen_pt[1][i]), &(s->linkf[dir[j]]), &(tempmat[i]));
