/*********************** io_lat4.c *************************/
/* MIMD version 7 */
/* THIS LOOKS LIKE AN ATTEMPT TO SWITCH THE x AND t ORDERING OF SITE
   DATA ON THE LATTICE.  THIS CODE IS NOT MAINTAINED! - CD 11/17/06 */
...
void w_serial(gauge_file *gf)
{
  ...
  for(j=0,x=0;x<nx;x++)for(z=0;z<nz;z++)for(y=0;y<ny;y++)for(t=0;t<nt;t++,j++)
    {
      ...
      if(this_node==0)
	{
	  ...
	  /* Pack tbufs in lbuf in the order t,y,z,x */
	  memcpy((void *)&lbuf[4*buf_length], 
		 (void *)(tbuf+3), sizeof(fsu3_matrix));
	  memcpy((void *)&lbuf[4*buf_length+1], 
		 (void *)(tbuf+1), 2*sizeof(fsu3_matrix));
	  memcpy((void *)&lbuf[4*buf_length+3], 
		 (void *)tbuf, sizeof(fsu3_matrix));
