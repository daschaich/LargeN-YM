...
//          (su3_matrix_f pm and su3_vector_f chi)
...
// diffmat is scratch space for an su3_matrix_f
// sumvec is scratch space for an su3_vector_f
...
void accum_gauge_hit(int gauge_dir, int parity) {
  ...
  FORSOMEPARITY(i, s, parity) {
    FORALLUPDIRBUT(gauge_dir, dir) {
      ...
      m1 = &(s->linkf[dir]);
      if (diffmat_offset >= 0)
        dif_su3_matrix_f(m1, (su3_matrix_f *)F_PT(s, diffmat_offset));
      else
        dif_su3_matrix_f(m1, &diffmatp[i]);
  ...
  FORSOMEPARITY(i, s, parity) {
    FORALLUPDIRBUT(gauge_dir, dir) {
      ...
      if (diffmat_offset >= 0)
        sum_su3_matrix_f(m2, (su3_matrix_f *)F_PT(s, diffmat_offset));
      else
        sum_su3_matrix_f(m2, &diffmatp[i]);
...
double get_gauge_fix_action(int gauge_dir, int parity) {
  ...
  FORSOMEPARITY(i, s, parity) {
    FORALLUPDIRBUT(gauge_dir, dir) {
      tc = trace_su3_f(&(s->linkf[dir]));
      gauge_fix_action += (double)tc.real;

      tc = trace_su3_f((su3_matrix_f *)gen_pt[dir][i]);
      gauge_fix_action += (double)tc.real;
    }
  }
