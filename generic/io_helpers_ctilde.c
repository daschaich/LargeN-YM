    ...
    if(prompt)  {
      ...
      while(s != 1){
        if(strcmp("bc_flag",tag) == 0 )  {
          printf("enter option for boundary values: 'trivial', or SU(2): 'alpha', or SU(3): 'alpha_one', or 'alpha_two' \n");
        }
        else if(strcmp("ct_flag",tag) == 0 )  {
          printf("enter option for c_t: 'tree_level', 'one_loop', or 'two_loop'\n");
        }
        else if(strcmp("ctilde_flag",tag) == 0 )  {
          printf("enter option for ctilde_t: 'tree_level', or 'one_loop'\n");
        }
        else {
...
/*        printf("= %d (%s)\n",*value,value_code); */
    ...
    else if(strcmp("ct_flag",tag) == 0 || strcmp("ctilde_flag",tag) == 0) {
