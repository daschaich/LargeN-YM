...
// Color vector
static void f2p_vec(vector *dest, fvector *src) {
  int i;
  for (i = 0; i < NCOL; i++) {
...
static void p2f_vec(fvector *dest, vector *src) {
  int i;
  for (i = 0; i < NCOL; i++) {
    dest->c[i].real = src->c[i].real;
    dest->c[i].imag = src->c[i].imag;
...
static void d2p_vec(vector *dest, dvector *src) {
  int i;
  for (i = 0; i < NCOL; i++) {
    dest->c[i].real = src->c[i].real;
    dest->c[i].imag = src->c[i].imag;
...
static void p2d_vec(dvector *dest, vector *src) {
  int i;
  for (i = 0; i < NCOL; i++) {
    dest->c[i].real = src->c[i].real;
    dest->c[i].imag = src->c[i].imag;
...
#if NCOL == 2
make_vget(F, 2, V, fvector, vector, p2f_vec);
make_vget(F, 2, M, fmatrix, matrix, p2f_mat);
make_vput(F, 2, V, fvector, vector, f2p_vec);
make_vput(F, 2, M, fmatrix, matrix, f2p_mat);
#endif
#if NCOL == 3
...
#if NCOL == 2
make_vget(D, 2, V, dvector, vector, p2d_vec);
make_vput(D, 2, V, dvector, vector, d2p_vec);
make_vput(D, 2, M, dmatrix, matrix, d2p_mat);
#endif
#if NCOL == 3
...
make_write_tslice(F, "F", 2, 2, 0, V, "USQCD_F2_ColorVector", fvector, vector, float);
#endif
#if NCOL == 3
...
make_write_tslice(D, "D", 2, 2, 0, V, "USQCD_D2_ColorVector", dvector, vector, double);
#endif
#if NCOL == 3
...
#if NCOL == 2
make_read(D, 2, V, dvector, vector, double);
make_read(D, 2, M, dmatrix, matrix, double);
#endif
#if NCOL == 3
